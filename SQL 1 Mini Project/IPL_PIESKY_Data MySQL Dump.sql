...
select isd.Stadium_Name,isd.City,count(ims.status) as'Number of matches' 
from ipl_match_schedule ims inner join ipl_stadium isd 
...
# 6. Display total matches played,total matches won and total matches lost by a team along with its team name.
select its.team_id,it.team_name,its.tournmt_id,sum(matches_played)Total_matches,sum(matches_won)Matches_Won,sum(matches_lost)Matches_lost 
from ipl_team_standings its
inner join ipl_team it on it.team_id=its.team_id 
group by its.team_id,it.team_name,its.tournmt_id;
# 7. Display the Bowlers for Mumbai Indians team.
select player_name,player_id 
from ipl_player  
where player_id in 
(select player_id from ipl_team_players where player_role = 'Bowler' and team_id = 5);
# 8. How many all rounders are there in each team, Display the team with more than 4 all rounder in descending order.
select itp.remarks,count(itp.player_id) as no_of_players 
